...
function Register() {
  ...
  const handleGoogleResponse = useCallback(
    async (response) => {
      ...
      try {
        await googleLogin(response.credential, true);
    ...
    [googleLogin, navigate, loading, googleLoading],
  );
  ...
  const handleStateChange = (e) => {
    ...
    if (selectedCountry === "NG") {
      const selected = nigeriaStates.find((item) => item.name === state);
  ...
  const validatePhone = () => {
    try {
      ...
      if (number.country !== selectedCountry) {
        return null;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.capital.trim()) {
      setError("Please enter your capital.");
      return;
    }
    ...
    const selectedCountryName =
      countryNames.of(selectedCountry) || selectedCountry;
          ...
          <div className="bg-linear-to-r from-[#3b2418] via-[#5a3825] to-[#8b5e3c] px-8 py-6 text-center">
            <Link to="/" className="inline-flex items-center gap-2 mb-2">
              ...
              <h1 className="font-['Georgia'] font-bold text-2xl text-white">
                Kings <span className="text-[#d9b99b]">Chops</span>
              </h1>
            </Link>
