...
const redirectToFrontend = (res, params = {}) => {
  const url = new URL("/order-history", FRONTEND_URL);
...
export const initializeKoraPayment = async (req, res) => {
  try {
    ...
    const secretKey = getKoraSecretKey();
    ...
    const redirectUrl = `${BACKEND_URL}/api/payments/kora/callback`;

    const notificationUrl = `${BACKEND_URL}/api/payments/kora/webhook`;
    ...
    const koraPayload = {
      ...
      customer: {
        name: customerName || "Kings Snacks Customer",
...
export const verifyKoraPayment = async (req, res) => {
  try {
    ...
    console.log("🔥 Kora verification status:", response.status);

    console.log("🔥 Kora verification response:", responseText);
    ...
    if (!response.ok || !data.status) {
      return res.status(400).json({
        ...
        message: data.message || "Unable to verify payment with Kora",
      });
    }
    ...
    console.log("🔥 Kora payment status:", payment?.status);
    ...
    const paidAmount = Number(payment?.amount_paid ?? payment?.amount);
...
export const handleKoraWebhook = async (req, res) => {
  try {
    ...
    if (!receivedSignature) {
      console.error("❌ Kora webhook signature missing");
    ...
    if (!req.body || !req.body.data) {
      console.error("❌ Kora webhook data missing");
    ...
    if (event !== "charge.success" && event !== "charge.failed") {
      console.log("ℹ️ Unsupported Kora event:", event);
    ...
    console.log("✅ Webhook order found:", order.orderCode);
    ...
    if (
      ...
      console.log(`❌ Order ${order.orderCode} marked as FAILED`);
    ...
    if (event !== "charge.success" || paymentStatus !== "success") {
      console.log("⏳ Webhook payment is not successful");
...
export const handleKoraCallback = async (req, res) => {
  try {
    ...
    const reference =
      req.query?.reference || req.query?.trxref || req.query?.payment_reference;
    ...
    if (!order) {
      console.error("❌ Callback order not found:", reference);
    ...
    console.log("✅ Callback order found:", order.orderCode);
    ...
    console.log("🔍 Verifying transaction directly with Kora...");
    ...
    console.log("🔥 Callback Kora HTTP status:", response.status);

    console.log("🔥 Callback Kora response:", responseText);
    ...
    try {
      ...
      console.error("❌ Invalid JSON returned by Kora");

      return redirectToFrontend(res, {
        payment: "failed",

        reason: "invalid_kora_response",
    ...
    if (!response.ok || !data.status) {
      console.error("❌ Kora callback verification failed:", data);
    ...
    console.log("🔥 Callback payment status:", payment?.status);

    console.log("🔥 Callback payment reference:", payment?.payment_reference);
    ...
    if (normalizedStatus !== "success") {
      console.log("⏳ Callback payment is not successful:", payment?.status);
    ...
    console.log("💰 Callback paid amount:", paidAmount);

    console.log("💰 Callback expected amount:", expectedAmount);

    if (paidAmount !== expectedAmount) {
      console.error("❌ Callback payment amount mismatch");
    ...
    console.error("❌ Kora callback error:", error);
